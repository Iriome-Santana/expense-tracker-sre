...
def test_add_expense(service, db):
    service.add_expense(db, "2026-01-27", "Coffee", 3.5, owner="test-user")
...
def test_add_expense_amount_zero(service, db):
    with pytest.raises(NegativeAmountError):
        service.add_expense(db, "2026-01-27", "Coffee", 0, owner="test-user")


def test_add_expense_empty_fields(service, db):
    with pytest.raises(FieldsRequiredError):
        service.add_expense(db, "", "", 0, owner="test-user")


def test_add_expense_invalid_date(service, db):
    with pytest.raises(ValueError):
        service.add_expense(db, "27-01-2026", "Coffee", 3.5, owner="test-user")


def test_show_expenses(service, db):
    expense = make_expense()
    db.query.return_value.filter.return_value.all.return_value = [expense]
    result = service.show_expenses(db, owner="test-user")
    assert len(result) == 1
...
def test_delete_expense_not_found(service, db):
    ...
    with pytest.raises(ExpenseNotFoundError):
        service.delete_expense(db, 999, owner="test-user")


def test_summary(service, db):
    expenses = [make_expense(amount="3.5"), make_expense(amount="1.5")]
    db.query.return_value.filter.return_value.all.return_value = expenses
    assert service.summary(db, owner="test-user") == 5.0


def test_summary_empty(service, db):
    db.query.return_value.filter.return_value.all.return_value = []
    assert service.summary(db, owner="test-user") == 0.0